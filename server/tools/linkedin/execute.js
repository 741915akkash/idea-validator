...
export default async function execute({ input, context }) {
  try {
    switch (input.action) {
      ...
      case 'company':
        return await company(input.input, context)
      ...
      default:
        return {
          ...
          warning: {
            code: 'UNKNOWN_ACTION',
            message: `Unknown LinkedIn action "${input.action}".`
          }
        }
    }
  } catch (error) {
...
async function search(input, context) {
  const result = await runTool({
    ...
    input: {
      query: `site:linkedin.com ${input.query}`
    },
...
async function company(input, context) {
  const browser = await runTool({
    toolId: 'browser',
