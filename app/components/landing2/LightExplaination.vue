...
<template>
  <section id="light-explanation-section" class="mx-auto max-w-4xl px-6 py-20">
    ...
    <div
      class="relative overflow-hidden rounded-3xl border border-slate-200 bg-white p-6 text-slate-900 shadow-sm sm:p-12"
    >
      <!-- Header -->
      <div
        class="mb-12 flex flex-col gap-2 border-b border-slate-200 pb-6 sm:grid sm:grid-cols-2 sm:gap-12"
      >
        ...
        <h3
          class="text-center text-sm font-bold uppercase tracking-[0.3em] text-slate-500 sm:pl-6 sm:text-left"
        >
          Real Example
        </h3>
      </div>
      ...
      <div class="flex flex-col">
        <div v-for="(step, index) in loopSteps" :key="step.id" class="group/row flex flex-col">
          <!-- Step Row -->
          <div
            class="-mx-4 grid grid-cols-1 items-stretch gap-1 rounded-xl px-4 transition-colors duration-300 hover:bg-slate-50 sm:grid-cols-2 sm:gap-12"
          >
            <!-- Left -->
            ...
            <!-- Right -->
            <div
              class="relative flex flex-col items-center justify-center border-l-0 border-slate-200 py-1.5 text-center text-sm font-medium leading-relaxed text-slate-700 sm:items-start sm:border-l sm:border-emerald-200 sm:py-3.5 sm:pl-6 sm:text-left sm:text-base"
            >
              <div
                class="absolute -left-[3px] top-1/2 hidden h-[6px] w-[6px] -translate-y-1/2 rounded-full bg-emerald-400 sm:block"
              />
          ...
          >
            ...
            <div class="hidden border-l border-slate-200 sm:ml-6 sm:block" />
          </div>
        </div>
        ...
        >
          ...
          <div
            class="flex items-center justify-center gap-3 rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm font-semibold text-emerald-700"
          >
            <div
              class="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-white text-slate-900 shadow-sm"
            >
              <ArrowRight class="h-3 w-3 text-emerald-600" />
            </div>
