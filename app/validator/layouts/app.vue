...
<template>
  <div class="flex min-h-screen bg-white font-sans">
    <Sidebar class="fixed left-0 top-0 hidden h-screen w-64 md:flex md:pt-10" />
    ...
    <div class="flex flex-1 flex-col md:ml-64">
      ...
      <main class="flex-1 px-4 pb-6 pt-2 md:hidden">
        <slot />
      </main>

      <main class="hidden min-w-0 flex-1 px-5 pb-6 pt-2 md:block">
        <slot />
      </main>
    </div>
  </div>
